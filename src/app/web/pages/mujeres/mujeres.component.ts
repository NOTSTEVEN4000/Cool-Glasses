...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, NgIf, NgStyle, RouterLink, GalleriaImagenesComponent, NgFor, NavBarComponent, FooterComponent],
  templateUrl: './mujeres.component.html',
  styleUrls: ['./mujeres.component.css']
})
export class MujeresComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.page === 'mujeres') {
      this.imagenes = this.imagenesService.getImagenes('mujeres');
    }
  }
  ...
  listarProductos(): void {
    this.subscription.add(
      this.ConexProductosService.getProductosByGenero('mujer').subscribe(
        (res: Producto[]) => {
