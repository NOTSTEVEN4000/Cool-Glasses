...
import {
  Favorito,
  FavoritoService,
} from '../../../core/services/favoritos/favoritos.service';
...
export class HombresComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.page === 'hombres') {
      this.imagenes = this.imagenesService.getImagenes('hombres');
    }
  }
  ...
  listarProductos(): void {
    this.subscription.add(
      this.ConexProductosService.getProductosByGenero('hombre').subscribe(
        (res: Producto[]) => {
  ...
  calcularPD(precio: number, descuento: number): string {
    ...
    return precioFinalRedondeado % 1 === 0
      ? `${precioFinalRedondeado.toFixed(2)}`
  ...
  applyFilters(): Producto[] {
    return (this.filteredProductos = this.listaProductos.filter((producto) => {
      const dentroDeRangoDesde =
        this.precioDesde != null
          ? this.calcularPD(producto.precio, producto.descuento) >=
            this.precioDesde
          : true;
      const dentroDeRangoHasta =
        this.precioHasta != null
          ? this.calcularPD(producto.precio, producto.descuento) <=
            this.precioHasta
          : true;
      const matchesColor =
        this.selectedColors.length > 0
          ? this.selectedColors.includes(producto.caracteristicas['color'])
          : true;
      const matchesDescuento = this.showDescuentoOnly
        ? producto.descuento > 0
        : true;
      const matchesNoDescuento = this.showNoDescuentoOnly
        ? producto.descuento === 0
        : true;
      const matchesCategory =
        this.selectedCategories.length > 0
          ? this.selectedCategories.includes(producto.categoria)
          : true;
      return (
        dentroDeRangoDesde &&
        dentroDeRangoHasta &&
